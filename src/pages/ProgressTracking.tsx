...
const STATUS_COLORS: Record<string, { bg: string; border: string; text: string; badge: string }> = {
    ...
    'Processing': { bg: 'bg-yellow-400/10', border: 'border-yellow-400/40', text: 'text-yellow-400', badge: 'bg-yellow-400/20 text-yellow-400 border-yellow-400/30' },
    ...
    'Not yet Acted': { bg: 'bg-slate-500/10', border: 'border-slate-500/40', text: 'text-muted-foreground', badge: 'bg-slate-500/20 text-muted-foreground border-slate-500/30' },
};

// Phase circle color based on completion
const getPhaseColor = (completed: boolean, isCurrent: boolean, status: string) => {
    if (!completed) return { circle: 'bg-card border-border', icon: 'text-slate-600', connector: 'bg-slate-700' };
                    ...
                    {allItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`text-sm font-medium ${isAttached ? 'text-foreground' : 'text-muted-foreground'}`}>
                                    {item.label}
                                </span>
                            </div>
                        );
                ...
                return (
                    <React.Fragment key={phase.key}>
                        ...
                        <div className="flex flex-col items-center flex-shrink-0 group relative px-1">
                            ...
                            `}>
                                <span className={`text-[10px] font-bold uppercase tracking-wider mb-0.5 ${completed ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
                                    {phase.shortLabel}
                                </span>
                                {dateVal && (() => {
                                    try {
                                        const d = new Date(dateVal);
                                        if (isNaN(d.getTime())) return <span className="text-[10px] font-mono text-muted-foreground bg-muted px-1 rounded">{dateVal}</span>;
                                        return <span className="text-[10px] font-mono text-muted-foreground bg-muted px-1 rounded">{format(d, 'MMM d')}</span>;
                                    } catch { return <span className="text-[10px] font-mono text-muted-foreground bg-muted px-1 rounded">{dateVal}</span>; }
                                })()}
                            </div>
                        </div>
...
const ProcurementCard = ({ procurement, onViewChecklist, onViewRecord }: { procurement: Procurement; onViewChecklist: (p: Procurement) => void; onViewRecord: (prNumber: string) => void }) => {
    ...
    return (
        <div className={`rounded-xl border ${colors.border} ${colors.bg} p-5 transition-all hover:shadow-xl hover:shadow-black/30 group`}>
            ...
            <div className="flex items-start justify-between mb-6">
                <div className="space-y-1">
                    <div className="flex items-center gap-3">
                        <h3 className="text-xl font-bold text-foreground font-mono tracking-tight">{procurement.prNumber}</h3>
                ...
                <div className="flex items-center gap-3">
                    <div className="text-right mr-4">
                        <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-0.5">Current Phase</p>
                    ...
                    <Button
                        ...
                        className="h-9 border-border bg-card text-muted-foreground hover:bg-accent hover:text-accent-foreground hover:text-foreground transition-colors"
                    >
                    ...
                    <Button
                        ...
                        className="h-9 border-border bg-blue-600/20 text-blue-300 hover:bg-blue-600/40 hover:text-foreground transition-colors"
                    >
            ...
            <div className="bg-background/50 rounded-xl p-4 border border-white/5 relative overflow-hidden">
    ...
    return (
        <div className="space-y-6 fade-in animate-in duration-500 pb-10">
            ...
            <div>
                ...
                <p className="text-muted-foreground mt-1">Visual pipeline of procurement phases for each record</p>
            </div>

            {/* Filters */}
            <div className="flex flex-wrap gap-3 p-4 bg-background rounded-xl border border-border sticky top-0 z-30 shadow-xl shadow-black/20 backdrop-blur-sm bg-opacity-90">
                {/* Search */}
            ...
            <div className="space-y-4">
                {paginated.length === 0 ? (
                    <div className="text-center py-24 text-muted-foreground bg-background/50 rounded-xl border border-border border-dashed">
                        <FileText className="h-16 w-16 mx-auto mb-4 opacity-20" />
                        <p className="text-xl font-medium text-muted-foreground">No records found</p>
            ...
            {filtered.length > 0 && (
                <div className="flex items-center justify-between pt-4 border-t border-border">
                    <p className="text-sm text-muted-foreground">
                        Showing <span className="text-foreground font-medium">{Math.min((safePage - 1) * ITEMS_PER_PAGE + 1, filtered.length)}–{Math.min(safePage * ITEMS_PER_PAGE, filtered.length)}</span> of <span className="text-foreground font-medium">{filtered.length}</span> Records
                    </p>
                    <div className="flex items-center gap-2">
                        ...
                        <Input
                            ...
                            placeholder="#"
                            className="w-14 h-9 text-center bg-card border-border text-foreground text-sm"
                        />
                        <Button
                            variant="outline"
                            size="sm"
                            ...
                            disabled={safePage <= 1}
                            className="h-9 border-border bg-card text-muted-foreground hover:bg-accent hover:text-accent-foreground hover:text-foreground disabled:opacity-40"
                        >
                            <ChevronLeft className="h-4 w-4 mr-1" />
                            Prev
                        </Button>
                        <span className="text-sm text-muted-foreground px-2 font-mono">{safePage} / {totalPages}</span>
                        <Button
                            variant="outline"
                            size="sm"
