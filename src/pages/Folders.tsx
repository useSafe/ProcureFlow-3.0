...
const ColorPicker: React.FC<{ color: string; onChange: (c: string) => void; idPrefix: string }> = ({ color, onChange, idPrefix }) => (
    <div className="col-span-3 space-y-3">
        ...
        <div className="flex items-center gap-2">
            <Label htmlFor={`${idPrefix}-color`} className="text-muted-foreground text-sm whitespace-nowrap">Custom:</Label>
            <div className="flex items-center gap-2 flex-1 min-w-0">
                <input id={`${idPrefix}-color`} type="color" value={color} onChange={e => onChange(e.target.value)}
                    className="h-9 w-14 rounded border-2 border-border bg-card cursor-pointer flex-shrink-0" />
                <Input type="text" value={color} onChange={e => onChange(e.target.value)} placeholder="#FF6B6B"
                    className="flex-1 bg-card border-border text-foreground font-mono text-sm min-w-0" />
            </div>
        </div>
    </div>
);
...
const ParentSelector: React.FC<{
    ...
    <>
        <div className="grid grid-cols-4 items-center gap-4">
            <Label className="text-right text-muted-foreground text-sm">Location</Label>
            <div className="col-span-3 flex gap-2">
                <button type="button" onClick={() => onParentTypeChange('shelf')}
                    className={cn('px-3 py-1.5 rounded text-sm font-medium transition-all', parentType === 'shelf' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-muted-foreground hover:text-white')}>
                    Drawer → Cabinet
                </button>
                <button type="button" onClick={() => onParentTypeChange('box')}
                    className={cn('px-3 py-1.5 rounded text-sm font-medium transition-all', parentType === 'box' ? 'bg-amber-600 text-white' : 'bg-slate-800 text-muted-foreground hover:text-white')}>
                    Inside Box
                </button>
            </div>
        </div>
        {parentType === 'shelf' ? (
            <>
                <div className="grid grid-cols-4 items-center gap-4">
                    <Label className="text-right text-muted-foreground text-sm">Drawer</Label>
                    <select value={selectedTier1Id} onChange={e => onTier1Change(e.target.value)}
                        className="col-span-3 bg-card border border-border text-white rounded-md p-2 text-sm">
                        <option value="">Select Drawer</option>
                        {cabinets.map(c => <option key={c.id} value={c.id}>{c.name} ({c.code})</option>)}
                    </select>
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                    <Label className="text-right text-muted-foreground text-sm">Cabinet</Label>
                    <select value={selectedTier2Id} onChange={e => onTier2Change(e.target.value)}
                        className="col-span-3 bg-card border border-border text-white rounded-md p-2 text-sm disabled:opacity-40"
            ...
            <div className="grid grid-cols-4 items-center gap-4 animate-in fade-in">
                <Label className="text-right text-muted-foreground text-sm">Box</Label>
        ...
        <div className="space-y-5 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    ...
                    <p className="text-muted-foreground mt-0.5 text-sm">Manage folders within cabinets and boxes</p>
                </div>
                    ...
                    <Dialog open={isAddDialogOpen} onOpenChange={o => { setIsAddDialogOpen(o); if (!o) resetForm(); }}>
                        ...
                        <DialogContent className="bg-background border-border text-foreground max-h-[90vh] overflow-y-auto">
                            <DialogHeader>
                                ...
                                <DialogDescription className="text-muted-foreground">Create a folder inside a cabinet or box.</DialogDescription>
                            </DialogHeader>
                            <div className="grid gap-4 py-4">
                                ...
                                />
                                <div className="grid grid-cols-4 items-center gap-4">
                                    <Label htmlFor="add-name" className="text-right text-muted-foreground text-sm">Name</Label>
                                    <Input id="add-name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Procurement 2024" className="col-span-3 bg-card border-border text-foreground" />
                                </div>
                                <div className="grid grid-cols-4 items-center gap-4">
                                    <Label htmlFor="add-code" className="text-right text-muted-foreground text-sm">Code</Label>
                                    <Input id="add-code" value={code} onChange={e => setCode(e.target.value)} placeholder="e.g. F1" className="col-span-3 bg-card border-border text-foreground" />
                                </div>
                                <div className="grid grid-cols-4 items-start gap-4">
                                    <Label className="text-right text-muted-foreground text-sm mt-2">Color</Label>
                                    <ColorPicker color={color} onChange={setColor} idPrefix="add" />
                                </div>
                                <div className="grid grid-cols-4 items-center gap-4">
                                    <Label htmlFor="add-desc" className="text-right text-muted-foreground text-sm">Description</Label>
                                    <Textarea id="add-desc" value={description} onChange={e => setDescription(e.target.value)} rows={2} className="col-span-3 bg-card border-border text-foreground" />
                                </div>
                            </div>
                            <DialogFooter>
                                <Button variant="outline" onClick={() => setIsAddDialogOpen(false)} className="border-border text-white hover:bg-muted">Cancel</Button>
                    ...
                    <div className="flex gap-2 items-center">
                        <div className="relative flex-1 max-w-sm min-w-0">
                            ...
                            <Input
                                ...
                                className="pl-9 pr-8 bg-card border-border text-foreground h-9 w-full"
                            />
                            {searchQuery && (
                        ...
                        <div className="flex items-center gap-1.5 ml-auto flex-shrink-0">
                            <Select value={sortField} onValueChange={v => setSortField(v as any)}>
                                <SelectTrigger className="w-[120px] bg-card border-border text-foreground h-9 text-xs gap-1">
                            ...
                            <Button variant="outline" size="sm"
                                ...
                                className="bg-card border-border text-foreground hover:bg-slate-700 h-9 w-9 p-0 flex-shrink-0"
                    ...
                    <div className="flex gap-2 items-center flex-wrap">
                        ...
                        <Select value={filterTier1Id || 'all'} onValueChange={handleFilterTier1Change}>
                            <SelectTrigger className={cn(
                                'h-8 text-xs border-border text-white w-[160px]',
                                activeDrawer ? 'bg-blue-500/10 border-blue-500/40' : 'bg-card'
                            )}>
                        ...
                        <Select value={filterCabinetId || 'all'} onValueChange={handleFilterCabinetChange}>
                            <SelectTrigger className={cn(
                                'h-8 text-xs border-border text-white w-[160px]',
                                activeCabinet ? 'bg-purple-500/10 border-purple-500/40' : 'bg-card',
                        ...
                        <Select value={filterBoxId || 'all'} onValueChange={handleFilterBoxChange}>
                            <SelectTrigger className={cn(
                                'h-8 text-xs border-border text-white w-[160px]',
                                activeBox ? 'bg-amber-500/10 border-amber-500/40' : 'bg-card',
                        ...
                        {hasActiveFilters && (
                            <button onClick={clearAllFilters}
                                className="text-xs text-muted-foreground hover:text-white underline underline-offset-2 transition-colors whitespace-nowrap ml-1">
                                Clear all
                            </button>
                        )}
                    </div>
            ...
            {/* ── Table Card ──────────────────────────────────────────────────── */}
            <Card className="border-none bg-background shadow-lg overflow-hidden">
                <CardHeader className="px-4 py-3 border-b border-border flex flex-row items-center justify-between space-y-0">
                    <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                        ...
                        Folder List
                        <Badge className="bg-slate-800 text-muted-foreground border-border font-normal text-xs ml-1">
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="border-border hover:bg-transparent">
                                    ...
                                    <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wide min-w-[180px]">Name</TableHead>
                                    <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wide min-w-[150px]">Parent</TableHead>
                                    <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wide w-[80px]">Code</TableHead>
                                    <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wide w-[110px]">Color</TableHead>
                                    <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wide text-center w-[65px]">Stack</TableHead>
                                    <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wide w-[70px]">Files</TableHead>
                                    <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wide text-right pr-4 w-[190px]">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {filteredFolders.length === 0 ? (
                                    ...
                                    filteredFolders.map(folder => {
                                        ...
                                        return (
                                            <TableRow key={folder.id} className={cn('border-border transition-colors', isSelected ? 'bg-blue-500/5' : 'hover:bg-card')}>
                                                ...
                                                <TableCell>
                                                    <div className="flex items-center gap-2.5">
                                                        ...
                                                        <div className="min-w-0">
                                                            ...
                                                            {folder.description && (
                                                                <p className="text-xs text-muted-foreground truncate max-w-[150px]" title={folder.description}>{folder.description}</p>
                                                            )}
                                                        </div>
                                                    </div>
                                                </TableCell>
                                                ...
                                                <TableCell>
                                                    {folder.boxId ? (
                                                        <div className="flex items-start gap-1.5 min-w-0">
                                                            ...
                                                            <div className="min-w-0">
                                                                <p className="text-[10px] text-muted-foreground leading-none mb-0.5">Box</p>
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="text-[10px] text-muted-foreground truncate max-w-[130px]" title={getParentDrawerName(folder.shelfId)}>{getParentDrawerName(folder.shelfId)}</p>
                                                            <p className="text-xs text-muted-foreground truncate max-w-[130px]" title={getParentCabinetName(folder.shelfId)}>↳ {getParentCabinetName(folder.shelfId)}</p>
                                                        </div>
                                                    )}
                                                </TableCell>
                                                ...
                                                {/* Code */}
                                                <TableCell>
                                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-mono bg-slate-800 text-muted-foreground border border-border">{folder.code}</span>
                                                </TableCell>
                                                ...
                                                <TableCell>
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-[10px] text-muted-foreground font-mono hidden xl:block">{folder.color || '#3b82f6'}</span>
                                                    </div>
                                                </TableCell>

                                                {/* Stack */}
                                                <TableCell className="text-center">
                                                    <span className="text-xs font-mono text-muted-foreground">{folder.stackNumber ? `#${folder.stackNumber}` : '—'}</span>
                                                </TableCell>
                                                ...
                                                <TableCell className="text-right pr-4">
                                                    <div className="flex justify-end items-center gap-0.5">
                                                        <Button variant="ghost" size="icon" onClick={() => handleRelocateClick(folder)}
                                                            className="h-8 w-8 text-muted-foreground hover:text-emerald-400 hover:bg-emerald-500/10" title="Update stack position">
                                                        ...
                                                        <Button variant="ghost" size="icon" onClick={() => handleEditClick(folder)}
                                                            className="h-8 w-8 text-muted-foreground hover:text-white hover:bg-slate-700">
                                                            ...
                                                            <AlertDialogContent className="bg-card border-border text-foreground">
                                                                ...
                                                                <AlertDialogFooter>
                                                                    ...
                                                                    <AlertDialogAction onClick={() => handleDelete(folder.id)} disabled={stats.files > 0}
                                                                        className={stats.files > 0 ? 'bg-slate-700 text-muted-foreground cursor-not-allowed' : 'bg-red-600 hover:bg-red-700 text-white'}>
                                                                        Delete
                                                                    </AlertDialogAction>
                                                                </AlertDialogFooter>
                                                            </AlertDialogContent>
            ...
            <Dialog open={isRelocateDialogOpen} onOpenChange={setIsRelocateDialogOpen}>
                <DialogContent className="bg-background border-border text-foreground">
                    ...
                    <div className="py-4">
                        <Label htmlFor="stackNumber" className="text-muted-foreground text-sm">Stack Number</Label>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsRelocateDialogOpen(false)} className="border-border text-white hover:bg-muted">Cancel</Button>
            ...
            <Dialog open={isEditDialogOpen} onOpenChange={o => { setIsEditDialogOpen(o); if (!o) resetForm(); }}>
                <DialogContent className="bg-background border-border text-foreground max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-muted-foreground">Update details for this folder.</DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        <ParentSelector
                            parentType={parentType} onParentTypeChange={handleParentTypeChange}
                        ...
                        />
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="edit-name" className="text-right text-muted-foreground text-sm">Name</Label>
                            <Input id="edit-name" value={name} onChange={e => setName(e.target.value)} className="col-span-3 bg-card border-border text-foreground" />
                        </div>
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="edit-code" className="text-right text-muted-foreground text-sm">Code</Label>
                            <Input id="edit-code" value={code} onChange={e => setCode(e.target.value)} className="col-span-3 bg-card border-border text-foreground" />
                        </div>
                        <div className="grid grid-cols-4 items-start gap-4">
                            <Label className="text-right text-muted-foreground text-sm mt-2">Color</Label>
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="edit-desc" className="text-right text-muted-foreground text-sm">Description</Label>
                            <Textarea id="edit-desc" value={description} onChange={e => setDescription(e.target.value)} rows={2} className="col-span-3 bg-card border-border text-foreground" />
                        </div>
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setIsEditDialogOpen(false)} className="border-border text-white hover:bg-muted">Cancel</Button>
