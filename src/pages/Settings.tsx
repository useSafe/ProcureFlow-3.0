...
import { updateUser, updateProcurement, getSuppliers } from '@/lib/storage';
import { useData } from '@/contexts/DataContext';
import { toast } from 'sonner';
import {
    Moon, Sun, Monitor, User, Lock, HelpCircle, MessageSquare,
    BookOpen, Send, Ticket, Eye, EyeOff, CheckCircle2, Shield,
    Bot, Loader2, FileText, Scale, AlertCircle, Camera, Trash2
} from 'lucide-react';
...
export default function Settings() {
    ...
    const handleMigrateSuppliers = async () => {
        ...
        try {
            ...
            for (const p of procurements) {
                if (!p.id) continue;
                if (p.supplier && typeof p.supplier === 'string' && p.supplier.trim() !== '') {
                    // Check if supplier is a known supplier ID
                    const matched = suppliers.find(s => s.id === p.supplier);
                    if (matched) {
                        try {
                            ...
                            console.error(`Failed to migrate ${p.id}:`, err);
                            errors++;
                        }
                    }
                }
            ...
            if (errors > 0) {
                toast.warning(`Migrated ${count} records, but ${errors} failed. Check console.`);
            } else {
                toast.success(`Successfully unencrypted ${count} procurement records.`);
            }
        } catch (error: any) {
            console.error('Migration error:', error);
            toast.error(`Migration failed: ${error?.message || error}`);
        } finally {
    ...
    const [profilePic, setProfilePic] = useState<string | null>(() => {
        return localStorage.getItem(`profile-pic-${user?.id}`) || null;
    });
    ...
    const handleProfilePicUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (ev) => {
            ...
            localStorage.setItem(`profile-pic-${user?.id}`, dataUrl);
    ...
    const handleRemoveProfilePic = () => {
        setProfilePic(null);
        localStorage.removeItem(`profile-pic-${user?.id}`);
        ...
        <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up">
            ...
            <Tabs defaultValue="appearance" className="space-y-4">
                ...
                <TabsContent value="profile">
                    <Card className="border border-border">
                        <CardHeader className="pb-3">
                            ...
                            <CardDescription className="text-xs">Update your display name, photo, and view account details.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="flex flex-col sm:flex-row gap-6 max-w-lg">
                                ...
                                <div className="flex flex-col items-center gap-3 shrink-0">
                                    <div className="relative group">
                                        <div className={cn(
                                            'h-24 w-24 rounded-full border-2 border-border overflow-hidden flex items-center justify-center text-3xl font-bold',
                                            !profilePic && 'bg-muted'
                                        )}>
                                            {profilePic ? (
                                                <img src={profilePic} alt="Profile" className="h-full w-full object-cover" />
                                            ) : (
                                                <span className="text-muted-foreground">{user?.name?.charAt(0)?.toUpperCase() || 'U'}</span>
                                            )}
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => fileInputRef.current?.click()}
                                            className="absolute -bottom-1 -right-1 h-8 w-8 rounded-full bg-foreground text-background flex items-center justify-center shadow-md hover:bg-foreground/80 transition-colors"
                                        >
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {profilePic && (
                                            <Button type="button" variant="ghost" size="sm" className="text-xs h-7 text-destructive hover:text-destructive" onClick={handleRemoveProfilePic}>
                                ...
                                {/* Form Column */}
                                <form onSubmit={handleSaveProfile} className="space-y-4 flex-1 min-w-0">
                ...
                <TabsContent value="security">
                    ...
                    {user?.role === 'admin' && (
                        <Card className="border border-red-500/30 mt-4 rounded-lg bg-red-900/5">
                            <CardHeader className="pb-2">
                                <CardTitle className="text-base text-red-400">Database Administration</CardTitle>
                                <CardDescription className="text-xs">Advanced database tools.</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <Button 
                                    type="button" 
                                    variant="destructive" 
                                    size="sm" 
                                    onClick={handleMigrateSuppliers}
                                    disabled={isMigrating}
                                >
                                    {isMigrating ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Bot className="h-4 w-4 mr-2" />}
                                    Unencrypt Suppliers (ID to Name)
                                </Button>
                                <p className="text-xs text-muted-foreground mt-2">
                                    This checks all records and replaces any Supplier ID with the actual Supplier Name for accurate display and counting.
                                </p>
                            </CardContent>
                        </Card>
                    )}
                    ...
                    <Card className="border border-border">
                        ...
                        <CardContent className="space-y-3">
                            {[
                                ...
                                { icon: Scale, title: 'IRR of RA 9184 Guide', desc: 'GOVERNMENT PROCUREMENT REFORM ACT Overview', href: 'https://www.gppb.gov.ph/wp-content/uploads/2024/07/Updated-2016-Revised-IRR-of-RA-No.-9184-as-of-19-July-2024.pdf' },
                                { icon: BookOpen, title: 'IRR of RA 12009 Guide', desc: 'NEW GOVERNMENT PROCUREMENT ACT Overview', href: 'https://www.gppb.gov.ph/wp-content/uploads/2025/02/IRR-of-RA-No.-12009.pdf' },
