...
export default function UrgentRecords() {
    ...
    const filteredRecords = useMemo(() => {
        ...
        const urgencyWeight: Record<string, number> = { 'High': 2, 'Low': 1, 'Done': 0, 'None': -1 };
        result.sort((a, b) => {
    ...
    const getUrgencyBadge = (level?: string) => {
        switch (level) {
            case 'High': return <Badge className="bg-red-500/15 text-red-400 border-red-500/30 font-semibold">🔴 High</Badge>;
            ...
            default: return <Badge className="bg-slate-700/60 text-muted-foreground border-border/60">—</Badge>;
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1 text-sm">Set urgency levels and action deadlines for procurement records.</p>
                </div>
                {/* Quick filter badges */}
                <div className="flex flex-wrap gap-2">
                    {(['High', 'Low', 'Done'] as UrgencyLevel[]).map(lvl => {
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    ...
                                    lvl === 'High' ? 'bg-red-500/15 text-red-400 border-red-500/30' :
                    ...
                    {filterUrgency !== 'all' && (
                        <button
                            ...
                            className="px-3 py-1 rounded-full text-xs font-semibold border border-border bg-slate-800 text-muted-foreground hover:text-white transition-all"
                        >
                            Clear ×
                        </button>
                    )}
                ...
                <CardHeader className="pb-3">
                    <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center w-full">
                        <CardTitle className="text-base flex items-center gap-2">
                            ...
                            Priority List
                            <span className="text-muted-foreground text-sm font-normal">({filteredRecords.length} records)</span>
                        </CardTitle>
                        <div className="flex flex-1 sm:flex-none gap-3 w-full sm:w-auto">
                            <div className="relative flex-1 sm:w-64">
                                ...
                                <Input
                                    ...
                                    className="pl-9 bg-background border-border text-foreground w-full h-9"
                                />
                            </div>
                            <Select value={filterUrgency} onValueChange={setFilterUrgency}>
                                <SelectTrigger className="w-[150px] bg-background border-border h-9 text-white">
                            ...
                            <tbody className="divide-y divide-border">
                                {filteredRecords.length === 0 ? (
                                    ...
                                    paginatedRecords.map((record) => {
                                        ...
                                        const previewDays = isEditing ? getDaysLeft(editDeadline ? format(editDeadline, 'yyyy-MM-dd') : undefined, editUrgency) : null;
                                        ...
                                        return (
                                            <tr key={record.id} className={cn(
                                                ...
                                                isEditing ? 'bg-muted/60 shadow-inner' : 'hover:bg-muted/50'
                                            )}>
                                                <td className="px-4 py-3 font-medium text-foreground whitespace-nowrap">{record.prNumber}</td>
                                                <td className="px-4 py-3 text-muted-foreground max-w-[180px] truncate" title={record.projectName}>
                                                    {record.projectName || '—'}
                                                </td>
                                                <td className="px-4 py-3 text-muted-foreground text-xs">{record.division || '—'}</td>

                                                {/* Urgency */}
                                                <td className="px-4 py-3">
                                                    {isEditing ? (
                                                        <Select value={editUrgency} onValueChange={(v: any) => setEditUrgency(v)}>
                                                            <SelectTrigger className="w-[130px] h-8 bg-background border-border text-foreground text-xs">
                                                                <SelectValue />
                                                            </SelectTrigger>
                                                ...
                                                <td className="px-4 py-3">
                                                    {isEditing ? (
                                                        <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
                                                            <PopoverTrigger asChild>
                                                                <Button
                                                                    variant="outline"
                                                                    className={cn(
                                                                        'h-8 w-[152px] justify-start gap-2 text-left text-xs font-normal bg-background border-border text-foreground hover:bg-muted hover:text-white',
                                                            ...
                                                            <PopoverContent
                                                                className="w-auto p-0 bg-card border-border shadow-2xl"
                                                                align="start"
                                                            >
                                                                <Calendar
                                                                    mode="single"
                                                                    ...
                                                                    initialFocus
                                                                    classNames={{
                                                                        day_today: "bg-slate-600 text-amber-400 font-bold border border-amber-500/30",
                                                        ...
                                                        <span className="text-slate-00 text-xs font-mono">
                                                    ...
                                                    <div className="flex gap-1.5 justify-end items-center">
                                                        {isEditing ? (
                                                            <>
                                                                ...
                                                                {isConfirmingReset ? (
                                                                    <div className="flex items-center gap-1 bg-muted/80 border border-border rounded-md px-2 py-1">
                                                                        ...
                                                                        <Button
                                                                            ...
                                                                            className="h-6 px-2 text-xs text-muted-foreground hover:text-white hover:bg-slate-700"
                                                                        >
                                                                            No
                                                                        </Button>
                                                                    </div>
                                                                ...
                                                                <Button
                                                                    ...
                                                                    title="Cancel"
                                                                    className="h-8 w-8 p-0 text-muted-foreground hover:text-white hover:bg-slate-700"
                                                                >
                                                                    <X className="w-4 h-4" />
                                                                </Button>
                                                            </>
